...
constexpr auto compile_shader(
  ...
  if (!shader_file) {
    std::println("[{}] [ERROR] Unable to read shader file: {}", current_time(),
  ...
  std::string const shader_source{std::istreambuf_iterator<char>(shader_file),
                                  std::istreambuf_iterator<char>()};

  // List of enabled compilation targets
  slang::TargetDesc const targetDesc{
      .format = SLANG_SPIRV,
      .profile = globalSession->findProfile("spirv_1_6")};

  // Create session descriptor
  // Note: create new local path object of file parent path due to lifetime of
  // c_str
  std::filesystem::path const module_search_path{
      shader_file_path.parent_path()};
  ...
  // Note: local session must live for as long as we need the linked Slang
  // shader program
  Slang::ComPtr<slang::ISession> session;
  if (SLANG_FAILED(
          globalSession->createSession(sessionDesc, session.writeRef()))) {
    std::println("[{}] [ERROR] Failed to create Slang local session.",
                 mazorca::current_time());
    return std::unexpected(mazorca::error_code::invalid);
  }

  // Load Slang module from provided input file path
  // Note: we import other Slang source files automatically into a single
  // translational unit
  Slang::ComPtr<slang::IModule> slangModule;
  {
    ...
    slangModule = session->loadModule(
        shader_file_path.stem()
            .c_str(),  // Module name (internal copy makes lifetime ok?)
    ...
    if (slangModule == nullptr) {
      std::println(
          "[{}] [ERROR] Failed to load Slang module into local session.",
          current_time());
      return std::unexpected(mazorca::error_code::invalid);
    }
  }

  // Identify number and names of entry points inside Slang module and store in
  // map object
  SlangInt32 const num_entry_points = slangModule->getDefinedEntryPointCount();
  std::unordered_map<std::string, Slang::ComPtr<slang::IEntryPoint>>
      entry_point_map;
  ...
  for (const auto& [name, entry] : entry_point_map) {
    ...
    Slang::ComPtr<slang::IComponentType> linked_program;
    {
      ...
      SlangResult const result =
          entry->link(linked_program.writeRef(), diagnosticsBlob.writeRef());
      if (diagnosticsBlob != nullptr) {
        std::println("{}", static_cast<const char*>(
  ...
  for (const auto& [name, program] : linked_program_map) {
    std::println("[{}] [INFO] Extracting SPIR-V blob for entry point name: {}",
                 mazorca::current_time(), name);
    ...
    {
      Slang::ComPtr<slang::IBlob> diagnosticsBlob;
      SlangResult const result = program->getEntryPointCode(
    ...
    std::println(
        "[{}] [INFO] Extracted {} bytes for program with entry point name: {}",
        mazorca::current_time(), spirv_blob->getBufferSize(), name);
