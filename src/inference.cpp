...
std::expected<void, mazorca::error_code> mazorca::grano::nn_example() {
    ...
    dnnl::graph::logical_tensor conv0_bias_add_dst_desc {
        4, 
    ...
    for (const auto& partition : partitions) {
        ...
        for (auto &input : inputs) {
            ...
            if (id_to_queried_logical_tensors.find(id)
                ...
                input = dnnl::graph::logical_tensor {id, input.get_data_type(),
                        concrete_shapes[id], dnnl::graph::logical_tensor::layout_type::strided};
        }

        // Update output logical tensors with concrete shape and layout
        for (auto &output : outputs) {
            const auto id = output.get_id();
            output = dnnl::graph::logical_tensor {id, output.get_data_type(),
                    DNNL_GRAPH_UNKNOWN_NDIMS, // set output dims to unknown
                    ids_with_any_layout.count(id) ? dnnl::graph::logical_tensor::layout_type::any
        ...
        allocate_graph_mem(inputs_ts, inputs, data_buffer,
                global_outputs_ts_map, engine, /*is partition input=*/true);
        allocate_graph_mem(outputs_ts, outputs, data_buffer,
                global_outputs_ts_map, engine, /*is partition input=*/false);
