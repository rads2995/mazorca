...
auto mazorca::app::run() const -> std::expected<void, mazorca::error_code> {
  mazorca::vulkan_data vulkan_data{.vk_allocator = nullptr,
  ...
  if (!SDL_Init(SDL_INIT_VIDEO)) {
    std::println("[{}] [ERROR] SDL_Init(): {}", mazorca::current_time(),
                 SDL_GetError());
  ...
  SDL_Window* window =
      SDL_CreateWindow("mazorca", static_cast<int>(1920 * main_scale),
                       static_cast<int>(1080 * main_scale), window_flags);

  if (window == nullptr) {
    std::println("[{}] [ERROR] SDL_CreateWindow(): {}", mazorca::current_time(),
                 SDL_GetError());
    return std::unexpected(mazorca::error_code::invalid);
  }
  ...
  VkResult err;
  if (static_cast<int>(SDL_Vulkan_CreateSurface(window, vulkan_data.vk_instance,
                                                vulkan_data.vk_allocator,
                                                &surface)) == 0) {
    std::println("[{}] [ERROR] Failed to create Vulkan surface.",
  ...
  if (SLANG_FAILED(slang::createGlobalSession(globalSession.writeRef()))) {
    std::println("[{}] [ERROR] Failed to create Slang global session.",
                 mazorca::current_time());
    return std::unexpected(mazorca::error_code::invalid);
  }
  ...
  for (const auto& grano : this->granos) {
    sycl_device_names.emplace_back(
        grano.sycl_device.get_info<sycl::info::device::name>());
  }
  ...
  while (!done) {
    ...
    if (fb_width > 0 && fb_height > 0 &&
        (vulkan_data.vk_swap_chain_rebuild ||
         vulkan_data.vk_main_window_data.Width != fb_width ||
...

    {
      ...
      ImGui::SameLine();
      ImGui::InputText("##sycl_path", input_kernel_bundle_file_path.data(),
      ...
      if (ImGui::Button("Compile SYCL kernel bundle")) {
        ...
        if (!kernel_bundle_file_path.empty()) {
          if (kernel_bundle_file_path.extension() != ".cpp") {
            kernel_bundle_status_message =
                "Invalid file extension! Valid entries include: .cpp";
            continue;
          }

          if (auto kernel_bundle =
                  this->granos[sycl_device_index].create_kernel_bundle(
                      kernel_bundle_file_path);
          ...
          kernel_bundle_status_message =
              "File path to SYCL kernel bundle is empty!";
        }
      }
...

    {
      ...
      static std::array<char, 256> input_shader_file_path{
          "/home/omega/Git/mazorca/shader/kernels.slang"};

      ImGui::Text("File path to shader file: ");
      ImGui::SameLine();
      ImGui::InputText("##slang_path", input_shader_file_path.data(),
      ...
      if (ImGui::Button("Compile shaders")) {
        ...
        if (!shader_file_path.empty()) {
          if (shader_file_path.extension() != ".slang") {
            shader_compiler_status_message =
                "Invalid file extension! Valid entries include: .slang";
            continue;
          }

          auto slang_program =
              mazorca::compile_shader(shader_file_path, globalSession);
  ...
  // Cleanup
  std::println("[{}] [INFO] Performing app clean-up and closing",
               mazorca::current_time());
