...
inline constexpr void check_vk_result(VkResult err) {
    if (err != VK_SUCCESS) {
        std::println("[vulkan] Error: VkResult = {}", std::to_underlying(err));
    }
}

[[nodiscard]] inline constexpr bool isExtensionAvailable(const ImVector<VkExtensionProperties>& properties, const char* extension) {
...
inline constexpr void CleanupVulkan(mazorca::vulkan_data& vulkan_data) {
...
inline constexpr void CleanupVulkanWindow(mazorca::vulkan_data& vulkan_data) {
    ImGui_ImplVulkanH_DestroyWindow(vulkan_data.vk_instance, vulkan_data.vk_device, &vulkan_data.vk_main_window_data, vulkan_data.vk_allocator);
}

inline constexpr void SetupVulkan(mazorca::vulkan_data& vulkan_data, ImVector<const char*> instance_extensions) {
    VkResult err;
    {
    ...
    if(vulkan_data.vk_physical_device == VK_NULL_HANDLE) {
        std::println("[{}] [ERROR] Vulkan opaque handle to physical device object points to null!", mazorca::current_time());
    }

    // Select graphics queue family
    vulkan_data.vk_queue_family = ImGui_ImplVulkanH_SelectQueueFamilyIndex(vulkan_data.vk_physical_device);
    IM_ASSERT(vulkan_data.vk_queue_family != static_cast<uint32_t>(-1));
    ...
    // Create Logical Device (with 1 queue)
    {
        ...
        err = vkCreateDevice(vulkan_data.vk_physical_device, &create_info, vulkan_data.vk_allocator, &vulkan_data.vk_device);
        check_vk_result(err);
    ...
    // If you wish to load e.g. additional textures you may need to alter pools sizes and maxSets.
    {
        ...
        err = vkCreateDescriptorPool(vulkan_data.vk_device, &pool_info, vulkan_data.vk_allocator, &vulkan_data.vk_descriptor_pool);
        check_vk_result(err);
    }
}
...
inline constexpr void SetupVulkanWindow(mazorca::vulkan_data& vulkan_data, ImGui_ImplVulkanH_Window* wd, VkSurfaceKHR surface, int width, int height) {
    wd->Surface = surface;

    // Check for WSI support
    VkBool32 res;
    vkGetPhysicalDeviceSurfaceSupportKHR(vulkan_data.vk_physical_device, vulkan_data.vk_queue_family, wd->Surface, &res);
    if (res != VK_TRUE) {
...
inline constexpr void FrameRender(mazorca::vulkan_data& vulkan_data, ImGui_ImplVulkanH_Window* wd, ImDrawData* draw_data) {
    VkSemaphore image_acquired_semaphore  = wd->FrameSemaphores[wd->SemaphoreIndex].ImageAcquiredSemaphore;
    VkSemaphore render_complete_semaphore = wd->FrameSemaphores[wd->SemaphoreIndex].RenderCompleteSemaphore;
    VkResult err = vkAcquireNextImageKHR(vulkan_data.vk_device, wd->Swapchain, UINT64_MAX, image_acquired_semaphore, VK_NULL_HANDLE, &wd->FrameIndex);
    ...
    {
        err = vkWaitForFences(vulkan_data.vk_device, 1, &fd->Fence, VK_TRUE, UINT64_MAX);    // wait indefinitely instead of periodically checking
        mazorca::check_vk_result(err);
...
inline constexpr void FramePresent(mazorca::vulkan_data& vulkan_data, ImGui_ImplVulkanH_Window* wd) {
    if (vulkan_data.vk_swap_chain_rebuild) return;
    VkSemaphore render_complete_semaphore = wd->FrameSemaphores[wd->SemaphoreIndex].RenderCompleteSemaphore;
    ...
    VkResult err = vkQueuePresentKHR(vulkan_data.vk_queue, &info);
    if (err == VK_ERROR_OUT_OF_DATE_KHR || err == VK_SUBOPTIMAL_KHR) {
        vulkan_data.vk_swap_chain_rebuild = true;
    }
    if (err == VK_ERROR_OUT_OF_DATE_KHR) {
